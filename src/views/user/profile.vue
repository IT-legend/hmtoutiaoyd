...
export default {
  data () {
    return {
      ...
      minDate: new Date(1990, 1, 1), // 生日组件 选择的最小日期
  ...
  methods: {
    ...
    openFileDialog () {
      this.$refs.myFile.click() // 触发input的click事件 触发事件就会弹出文件对话框
    },
    // 封装方法
    ...
    selectItem (item, index) {
      ...
      // index 0男 1女
      // 通过iem或者index可以得到点击的是男还是女
    },
    // 显示生日弹层
    showDate () {
      this.showBirthDay = true // 显示生日弹层
      // 将当前的生日 设置到 选择日期的当前时间 将生日字符串 转化为Date对象 绑定到 日期组件上
    ...
    confirmDate () {
      ...
      this.user.birthday = dayjs(this.currentDate).format('YYYY-MM-DD') // 将date类型转化为字符串
    ...
    async upload (params) {
      ...
      data.append('photo', this.$refs.myFile.file[0]) // 第二个参数 是选择的图片文件 选择图片文件即可
